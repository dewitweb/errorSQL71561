CREATE VIEW [sub].[viewStatistics_Processed_Declarations_2019]
AS
...
(
	SELECT	dep.DeclarationID,
			SUM(dep.PartitionAmountCorrected)	AS SumPartitionAmountCorrected
	FROM	sub.tblDeclaration_Partition dep
	INNER JOIN sub.tblDeclaration decl ON decl.DeclarationID = dep.DeclarationID
	WHERE	((decl.SubsidySchemeID = 1 AND PartitionYear >= '2019')
		OR	 decl.SubsidySchemeID <> 1)
	AND		dep.PartitionStatus IN ('0010', '0012', '0014', '0016', '0017')
	GROUP BY 
			dep.DeclarationID
),
