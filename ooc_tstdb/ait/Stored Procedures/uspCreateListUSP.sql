...
CREATE PROCEDURE [ait].[uspCreateListUSP] 
			...
			CASE	WHEN t.name = 'nvarchar' 
					...
					ELSE 
						CASE	WHEN t.name = 'varchar' 
								...
								END 
							END AS ColumnLength, 
			ROW_NUMBER() OVER(ORDER BY C.Column_ID) Column_ID, LEN(C.Name) AS LengthName
			FROM	sys.tables Tab
...
SET @UspName = 'usp'+ SUBSTRING(@TableName, 4, LEN(@TableName)) + '_List'
...
--SELECT	CHAR(9) + '@' + ColumnName + char(9) + TypeName + 
...
INSERT INTO @Lines VALUES ('/*	==========================================================================================')
INSERT INTO @Lines VALUES (CHAR(9) + 'Purpose: ' + CHAR(9) + 'Get list from ' + @SchemaName + '.' + @TableName + '.')
INSERT INTO @Lines VALUES ('')
...
INSERT INTO @Lines VALUES ('ORDER BY ' + @OrderBy)
